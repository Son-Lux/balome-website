...
switch ($data['action']) {
    ...
    case 'publish_article':
        ...
        // Aggiorna le card news in index.html
        aggiorna_news_index($articoli);
...
// ── Aggiorna sezione news in index.html ─────────────────────────
function aggiorna_news_index(array $articoli): void {
    ...
    // Prendi i 3 articoli più recenti
    $ultimi = array_slice($articoli, 0, 3);

    // Costruisci le card HTML
    $cards = '';
    foreach ($ultimi as $a) {
        $titolo   = htmlspecialchars($a['titolo'] ?? '', ENT_QUOTES);
        $estratto = htmlspecialchars($a['estratto'] ?? '', ENT_QUOTES);
        // Tronca estratto a 160 caratteri
        if (mb_strlen($estratto) > 160) {
            $estratto = mb_substr($estratto, 0, 160) . '…';
        }
        ...
        $dataStr = '';
        if (!empty($a['data'])) {
            [$y, $m, $d] = explode('-', $a['data']);
            $dataStr = (int)$d . ' ' . $mesi[(int)$m] . ' ' . $y;
        }
...
HTML;
    ...
    // Sostituisci il blocco tra i marker
    $content = file_get_contents($indexPath);
    $pattern = '/(<!-- NEWS:START -->).*?(<!-- NEWS:END -->)/s';
    $replacement = "<!-- NEWS:START -->{$cards}\n        <!-- NEWS:END -->";
